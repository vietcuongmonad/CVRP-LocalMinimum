''' In this problem scope
    ...
    Assert random max 5% package rate to vehicle
'''
...
def algorithm():
    ...
    random.seed()
    num_assert = max(1, int(N * assert_rate))

    item_order = list(range(N))
    random.shuffle(item_order)

    car_order = list(range(M))
    random.shuffle(car_order)

    for i in range(num_assert):
        for j in range(M):
            delta, tmp_s, tmp_f = tryout(item_order[i], car_order[j])

            if (delta >= 0):
                sum_mileage += delta
                exec(item_order[i], car_order[j], tmp_s, tmp_f)
                break
